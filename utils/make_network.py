...
def nm_suppression(boxes, scores, overlap=0.45, top_k=200):
    ...
    # boxesと同じ torch.dtypeとtorch.deviceを持つテンソルの作成
    tmp_x1 = boxes.new()
    tmp_y1 = boxes.new()
    tmp_x2 = boxes.new()
    tmp_y2 = boxes.new()
...
class Detect(Function):
    @staticmethod
    def forward(ctx, loc_data, conf_data, dbox_list, conf_thresh = 0.01, top_k = 200, nms_thresh = 0.45):
        """
        順伝搬の計算を実行する。
        ...
        ctx.softmax = nn.Softmax(dim=-1)  # confをソフトマックス関数で正規化するために用意
        ctx.conf_thresh = conf_thresh  # confがconf_thresh=0.01より高いDBoxのみを扱う
        ctx.top_k = top_k  # nm_supressionでconfの高いtop_k個を計算に使用する, top_k = 200
        ctx.nms_thresh = nms_thresh  # nm_supressionでIOUがnms_thresh=0.45より大きいと、同一物体へのBBoxとみなす
        ...
        for i in range(num_batch):
            ...
            for cl in range(1, num_classes):
                ...
                ids, count = nm_suppression(boxes, scores, ctx.nms_thresh, ctx.top_k)
